...
export const createProject = async (req, res) => {
  try {
    ...
    // Create project WITH a default subproject in one go
      ...
      subprojects: [
        {
          ...
          // (status: "In Progress", designState, openingOption, quantity, …)
        },
      ],
    });
    ...
    res.status(201).json({
      ...
      project, // project.subprojects[0]._id is now available
    });
  } catch (error) {
...
// ✅ NEW: Save design state to subproject
// PATCH /api/projects/:id/subprojects/:subId/design-state
export const saveDesignState = async (req, res) => {
  try {
    const { id: projectId, subId: subprojectId } = req.params;
    const { designState } = req.body;

    if (!designState) {
      return res.status(400).json({
        success: false,
        message: "Design state is required",
      });
    }
    ...
    // Update design state
    subproject.designState = designState;
    subproject.updatedAt = Date.now();
    project.updatedAt = Date.now();
    ...
    res.status(200).json({
      success: true,
      message: "Design state saved successfully",
      designState: subproject.designState,
    });
  } catch (error) {
    console.error("Save design state error:", error);
    res.status(500).json({
      success: false,
      message: "Server error saving design state",
    });
  }
};

// ✅ NEW: Get design state from subproject
// GET /api/projects/:id/subprojects/:subId/design-state
export const getDesignState = async (req, res) => {
  try {
    const { id: projectId, subId: subprojectId } = req.params;
    ...
    res.status(500).json({
      success: false,
      message: "Server error fetching design state",
    });
  }
};