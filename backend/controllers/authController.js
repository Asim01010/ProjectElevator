...
console.log("EMAIL_PASS length:", process.env.EMAIL_PASS?.length);
    ...
    try {
      ...
      console.error("⚠️ OTP email failed to send:", mailErr.message);
      // User record already exists — don't fail the whole request.
      // Let them proceed to the OTP screen and use "Resend OTP" once email is fixed.
      return res.status(201).json({
        success: true,
        message:
          "Account created, but the verification email could not be sent right now. Please use 'Resend verification code' on the next screen.",
        userId: user._id,
        email: user.email,
      });
    }
...
export const resendOTP = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("⚠️ Resend OTP email failed:", mailErr.message);
      return res.status(502).json({
        success: false,
        message:
          "Could not send the email right now. Please check the server's email configuration and try again.",
      });
    }
